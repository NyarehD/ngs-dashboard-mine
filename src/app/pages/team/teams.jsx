                ...
                <TableBody className={teamStyle.tableBody}>
                    {items.map(row => (
                        <TableRow sx={{maxHeight: '100px'}} key={row.id}>
                            ...
                            <TableCell
                                scope="row">{all ? row.team.charAt(0).toUpperCase() + row.team.slice(1) : row.phone}</TableCell>
                            ...
                            <TableCell scope="row" className={teamStyle.actions}>
                                <IconButton
                                    onClick={_ => navigate(`/team/${row.team}/${row.id}/edit`, {state: {row, teams}})}
                                    aria-label="delete">
...
export default function Teams() {
    ...
    return (
        <Container maxWidth={false}
                   className={`${teamStyle.teamContainer} ${darkMode.mode === "dark" && teamStyle.darkMode}`}>
                ...
                <Grid item container md={5} justifyContent="space-between">
                    <Grid item xs={8} container>
                        {addingTeam ?
                            <>
                                <Grid item xs={10}>
                                    <TextField fullWidth value={actionTeamName}
                                               onChange={e => setActionTeamName(e.target.value)} className={teamStyle.addTeam}></TextField>
                                </Grid>
                                <Grid item xs={5}>
                                    <Button color='error' onClick={_ => setAddingTeam(false)}
                            ...
                            <FormControl fullWidth>
                                ...
                                <Select
                                    ...
                                    sx={darkMode.mode === "dark" ? {background: "#bebebe"} : {}}
                                >
                                    ...
                                    <Button onClick={_ => AddTeam(false)} fullWidth color='primary'
                                            className={`${darkMode.mode === 'dark' ? teamStyle.teamSelectItemDark : teamStyle.teamSelectItem}`}>Add
                                        Team&nbsp;<FontAwesomeIcon
                                            size="sm" icon={faPlus}/></Button>
                                    <Button onClick={_ => AddTeam(false, true)} fullWidth color='primary'
                                            className={`${darkMode.mode === 'dark' ? teamStyle.deleteTeamDark : teamStyle.deleteTeam}`}>Delete
                                        Team&nbsp;<FontAwesomeIcon size="sm"
                                                                   color="red"
